...
def togglestate(topic, payload, section, srv):
    ...
    try:
        ...
        if topic in toggles:
            ...
            srv.logging.debug(
                "togglestate() was called from the {} section and found {} in {}".format(section, topic, filename)
            )
        else:
            ...
            srv.logging.debug(
                "togglestate() was called from the {} section, did not find {} in {}".format(section, topic, filename)
            )
            srv.logging.debug(
                "togglestate() added {} to {} with blocking set to {}".format(topic, filename, topicblock)
            )
